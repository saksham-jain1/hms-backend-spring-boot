...
public interface BookingsMapper {
    ...
    @Select("SELECT * from bookings where hotelId ${hotelId} and month(checkOut) = ${month} and year(checkout) = ${year} Limit ${Limit}")
    List<Bookings> getBookingsAll(String hotelId, int month, int year, String Limit);
    ...
    @Update("Update bookings set checkIn = null where id = #{id}")
    void cancel(int id);
